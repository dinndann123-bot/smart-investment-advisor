...
# Live Discovery v6.1: dynamic universe + PM/VWAP/RVOL + explicit provenance.
...
try:
 ...
 STRATEGY_VERSION='strategy-learning-v6.1-pm-rvol-provenance'
 UNIVERSE_MODE='dynamic_alpaca_pm_rvol_v6_1'
 def _f(v,d=0.0):
  try:
 ...
 def _features(r,bars):
  ...
  firstpm=_f(pm[0][1].get('o')) if pm else 0;gap=(firstpm/prev-1)*100 if firstpm and prev else _f(r.get('change_pct'));hist=[]
  try:
   from zoneinfo import ZoneInfo
  ...
  if not(ALPACA_KEY and ALPACA_SECRET):payload={'results':[],'error':'alpaca_not_configured'}
   ...
   for a in assets:
    if sn.get(a['ticker']):
     ...
     if r:pool.append(r)
   pool.sort(key=lambda x:(_f(x.get('day_volume'))*_f(x.get('price')))*(1+min(abs(_f(x.get('change_pct'))),8)/20),reverse=True);pool=pool[:180];sem=asyncio.Semaphore(8);enriched=[]
   async def one(r):
    async with sem:enriched.append(_features(r,await _bars(r['ticker'])))
   await asyncio.gather(*(one(r) for r in pool))
   for r in enriched:
    rank,m=_rank(r);r.update({'score':max(0,min(100,round(rank))),'forward_rank':round(rank,2),'gate_metrics':m,'candidate_type':'prediction','prediction_status':'pm_rvol_pre_move_candidate','strategy_version':STRATEGY_VERSION,'scan_id':scan_id,'generated_at':generated.isoformat(),'universe_mode':UNIVERSE_MODE})
   enriched.sort(key=lambda x:x.get('forward_rank',-999),reverse=True);sel=enriched[:wanted]
   payload={'results':sel,'feed':ALPACA_FEED,'data_source':'Alpaca','assets_scanned':len(assets),'enriched_candidates':len(enriched),'requested_top':wanted,'complete_top10':len(sel)>=wanted,'ranking_status':'experimental_learning_overlay','note_he':'v6.1 מוסיף חתימת provenance לכל סריקה; אין שינוי במשקלי הדירוג של v6.'}
 ...
 if _day_route:_day_route.endpoint=_scanner;SCANNER_UNIVERSE_ALIGNMENT={'installed':True,'mode':UNIVERSE_MODE,'strategy_version':STRATEGY_VERSION,'target_count':10,'ui_untouched':True,'historical_universe_untouched':True,'experimental':True,'cache_policy':'no-store'}
...
try:
 ...
 LEARNING_ENGINE_STATUS={'installed':True,'strategy_version':'strategy-learning-v6.1-pm-rvol-provenance','stable_base':'1b8068c52a5f7ba5ab6ee455999330b102dbc90a','universe_alignment':SCANNER_UNIVERSE_ALIGNMENT,'feature_comparison':True}
