...
try:
    ...
    async def _aligned_day_scanner(top:int=10,candidates:int=40):
        ...
        out=dict(base or {}); out['results']=observable[:wanted]; out['feed']=ALPACA_FEED; out['observable_count']=len(observable); out['unobservable_count']=len(rejected); out['unobservable_candidates']=rejected[:20]; out['universe_alignment']='discovery_broad_promotion_configured_feed'; out['requested_top']=wanted; return out
    if _day_route:
        _day_route.endpoint=_aligned_day_scanner; SCANNER_UNIVERSE_ALIGNMENT={'installed':True,'feed':ALPACA_FEED,'mode':'broad_discovery_verified_promotion'}
...
async def _delayed_learning_evaluation(delay_seconds=95):
    # A signal captured in the middle of a 1-minute candle cannot be evaluated
    # immediately without either getting zero bars or contaminating the result with
    # pre-capture price action. Wait for the first full post-capture minute instead.
    ...
    try:
        ...
        print(f'LEARNING_EVALUATE_DELAYED ok={ev.get("ok")} requested={ev.get("requested")} evaluated={ev.get("evaluated")} complete={ev.get("complete")} partial={ev.get("partial")} delay={delay_seconds}',flush=True)
