...
# Live Discovery v6: dynamic universe + point-in-time PM/VWAP/RVOL features.
# Historical research and UI are untouched. Ranking remains experimental.
try:
 ...
 def _basic(meta,s):
  ...
  ch=(p/prev-1)*100
  return {**meta,'price':round(p,4),'prev_close':prev,'change':round(ch,2),'change_pct':round(ch,2),'day_volume':vol,'market_timestamp':lt.get('t') or mb.get('t') or db.get('t'),'data_source':'Alpaca','data_feed':ALPACA_FEED,'data_verified':True,'stale':False}
 ...
 def _features(r,bars):
  ...
  todays=byday.get(today,[]);prev=_f(r.get('prev_close'));pm=[];regular=[]
  # Alpaca timestamps are UTC. US premarket is 08:00-13:30 UTC during EDT;
  # use NY conversion when zoneinfo is available to avoid DST assumptions.
  try:
   ...
   for t,b in todays:
    ...
    elif 570<=mins<960:regular.append((t,b))
  except:pass
  pmv=sum(_f(b.get('v')) for _,b in pm);pmh=max([_f(b.get('h')) for _,b in pm] or [0]);pml=min([_f(b.get('l'),1e99) for _,b in pm] or [0]);pv=sum(_f(b.get('v'))*_f(b.get('vw'),_f(b.get('c'))) for _,b in pm);pmvwap=pv/pmv if pmv else 0
  firstpm=_f(pm[0][1].get('o')) if pm else 0;gap=(firstpm/prev-1)*100 if firstpm and prev else _f(r.get('change_pct'))
  # Same-clock historical volume baseline: compare cumulative volume through the
  # current local NY clock against prior sessions through that same clock.
  hist=[]
  try:
   from zoneinfo import ZoneInfo
  ...
  baseline=(sum(hist[-7:])/len(hist[-7:])) if hist else 0;todaycum=sum(_f(b.get('v')) for t,b in todays if True);rvol=todaycum/baseline if baseline else 0
  price=_f(r.get('price'));distv=(price/pmvwap-1)*100 if pmvwap else 0;dist_high=(price/pmh-1)*100 if pmh else 0
  r.update({'premarket_volume':round(pmv),'premarket_high':round(pmh,4) if pmh else None,'premarket_low':round(pml,4) if pml else None,'premarket_vwap':round(pmvwap,4) if pmvwap else None,'premarket_gap_pct':round(gap,2),'rvol':round(rvol,2) if rvol else None,'rvol_baseline_sessions':min(len(hist),7),'distance_from_pm_vwap_pct':round(distv,2) if pmvwap else None,'distance_from_pm_high_pct':round(dist_high,2) if pmh else None})
  return r
 def _rank(r):
  ch=_f(r.get('change_pct'));gap=_f(r.get('premarket_gap_pct'));rv=_f(r.get('rvol'));pmv=_f(r.get('premarket_volume'));dv=_f(r.get('distance_from_pm_vwap_pct'));dh=_f(r.get('distance_from_pm_high_pct'))
  participation=min(max(rv-1,0),9)*4+min(pmv/100000,15);gap_signal=min(max(gap,0),7)*1.5;structure=(4 if -3<=dv<=5 else 0)+(4 if -8<=dh<=1 else 0);extension=max(ch-6,0)*2.8+max(gap-10,0)*2
  if ch>=12:extension+=10
  if ch>=20:extension+=18
 ...
 async def _scanner(top:int=10,candidates:int=40):
  ...
  if not(ALPACA_KEY and ALPACA_SECRET):return {'results':[],'error':'alpaca_not_configured'}
  assets=await _assets();sn=await _snapshots([a['ticker'] for a in assets]);pool=[]
  for a in assets:
   if sn.get(a['ticker']):
    r=_basic(a,sn[a['ticker']])
    if r:pool.append(r)
  # Cost control: enrich a broad but finite discovery pool. Selection is based on
  # liquidity + moderate current move, not simply the largest gainers.
  pool.sort(key=lambda x:(_f(x.get('day_volume'))*_f(x.get('price')))*(1+min(abs(_f(x.get('change_pct'))),8)/20),reverse=True);pool=pool[:180]
  sem=asyncio.Semaphore(8);enriched=[]
  async def one(r):
   async with sem:
    b=await _bars(r['ticker']);enriched.append(_features(r,b))
  await asyncio.gather(*(one(r) for r in pool))
  for r in enriched:
   rank,m=_rank(r);r.update({'score':max(0,min(100,round(rank))),'forward_rank':round(rank,2),'gate_metrics':m,'candidate_type':'prediction','prediction_status':'pm_rvol_pre_move_candidate'})
  enriched.sort(key=lambda x:x.get('forward_rank',-999),reverse=True);sel=enriched[:wanted]
  return {'results':sel,'feed':ALPACA_FEED,'data_source':'Alpaca','server_timestamp':datetime.now(timezone.utc).isoformat(),'assets_scanned':len(assets),'enriched_candidates':len(enriched),'universe_alignment':'dynamic_alpaca_pm_rvol_v6','candidate_semantics':'ten_forward_candidates_not_top_movers','requested_top':wanted,'complete_top10':len(sel)>=wanted,'ranking_status':'experimental_learning_overlay','note_he':'v6 מוסיף PM volume/high/low/VWAP ו-RVOL לפי נפח מצטבר מול אותה שעה בסשנים קודמים. Catalyst/float/SEC עדיין לא משוקללים עד מקור מאומת.'}
 if _day_route:_day_route.endpoint=_scanner;SCANNER_UNIVERSE_ALIGNMENT={'installed':True,'mode':'dynamic_alpaca_pm_rvol_v6','target_count':10,'ui_untouched':True,'historical_universe_untouched':True,'experimental':True}
...
try:
 ...
 LEARNING_ENGINE_STATUS={'installed':True,'strategy_version':'strategy-learning-v6-pm-rvol','stable_base':'1b8068c52a5f7ba5ab6ee455999330b102dbc90a','universe_alignment':SCANNER_UNIVERSE_ALIGNMENT,'feature_comparison':True}
