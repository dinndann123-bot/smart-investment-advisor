...
function firstNum(...v){for(const z of v){const n=+z;if(Number.isFinite(n))return n}return null}
function normalize(x,i,scanId){
 const price=firstNum(x.price,x.latest_price,x.current_price);
 const prev=firstNum(x.previous_close,x.prev_close,x.previousClose);
...
function validate(j){
 ...
 const seen=new Set();
 for(let i=0;i<10;i++){const x=j.results[i],t=String(x?.ticker||'').trim().toUpperCase();if(!t)return `ticker missing at rank ${i+1}`;if(seen.has(t))return `duplicate ticker ${t}`;seen.add(t);if(x.breakout_stage==='already_extended'||x.candidate_type==='learning_observation')return `non-predictive row ${t}`;if(x.scan_id&&x.scan_id!==j.scan_id)return `scan_id mismatch ${t}`}
 return null;
}
function invalidate(message){window.dayData=[];state.status='error';state.error=message;state.scanId=null;state.generatedAt=null;state.count=0;state.integrity=false;window.__V5_DAY_SCAN_PAYLOAD__=null;emit({integrity_error:message});console.error('DAY_TOP10_INTEGRITY_FAIL',message)}
async function scanDay({force=false}={}){
 ...
 }catch(e){if(my===seq)invalidate(String(e?.message||e));throw e}finally{if(my===seq)active=null}})();return active;
}
window.v5DayScanner={scan:scanDay,state:()=>({...state}),validate};window.addEventListener('smartadvisor:request-day-scan',()=>scanDay({force:true}).catch(()=>{}));
