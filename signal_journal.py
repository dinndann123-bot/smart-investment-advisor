    ...
    async with httpx.AsyncClient(timeout=25) as client:
        for s in symbols:
            try:
                ...
                out[s]={'premarket_price':_f(bars[-1].get('c')),'premarket_volume':sum(_f(b.get('v')) or 0 for b in bars),'premarket_high':max(_f(b.get('h')) or 0 for b in bars) or None,'premarket_low':min(lows) if lows else None}
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for row in rows:
            try:
                ...
                safe_now=now-timedelta(seconds=5); close_utc=regular_close.astimezone(timezone.utc); start_utc=start.astimezone(timezone.utc); query_end=min(safe_now,close_utc)
                if query_end<=start_utc:query_end=min(start_utc+timedelta(minutes=1),close_utc)
                bars=await _bars(core,client,row['symbol'],start_utc.isoformat(),query_end.isoformat())
                ...
                peak_px=_f(peak.get('h')); trough_px=_f(trough.get('l')); close_px=_f(last.get('c')); pt=_iso(peak.get('t')); tt=_iso(trough.get('t')); exit_px=_f(peak.get('c')) or peak_px; complete=now>=close_utc
