...
async def _bars(core,client,symbol,start,end):
    ...
    h={'APCA-API-KEY-ID':core.ALPACA_KEY,'APCA-API-SECRET-KEY':core.ALPACA_SECRET};p={'timeframe':'1Min','start':start,'end':end,'limit':10000,'feed':core.ALPACA_FEED,'adjustment':'split','sort':'asc'}
    r=await client.get(f'https://data.alpaca.markets/v2/stocks/{symbol}/bars',headers=h,params=p)
...
async def _premarket(core,symbols):
    ...
    async with httpx.AsyncClient(timeout=25) as client:
        for s in symbols:
            try:
                bars=await _bars(core,client,s,f'{day}T04:00:00-04:00',f'{day}T09:30:00-04:00')
...
async def _evaluate_rows(core,rows):
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for row in rows:
            try:
                captured=_iso(row['captured_at']);scanner_entry=_f(row['price'])
                if not captured or not scanner_entry or scanner_entry<=0:continue
                ny=captured.astimezone(NY);regular_open=ny.replace(hour=9,minute=30,second=0,microsecond=0);regular_close=ny.replace(hour=16,minute=0,second=0,microsecond=0)
                # Captures can occur mid-minute. Alpaca minute bars are timestamped at the minute boundary,
                # so floor the query start to that boundary and then use the first bar close as executable proxy.
                start=max(regular_open,ny.replace(second=0,microsecond=0));safe_now=now-timedelta(seconds=10);close_utc=regular_close.astimezone(timezone.utc);start_utc=start.astimezone(timezone.utc);query_end=min(safe_now,close_utc)
                if start>=regular_close or query_end<=start_utc:continue
                bars=await _bars(core,client,row['symbol'],start_utc.isoformat(),query_end.isoformat());bars=[b for b in bars if _f(b.get('h')) is not None and _f(b.get('l')) is not None and _f(b.get('c')) is not None]
                if not bars:continue
                first=bars[0];entry=_f(first.get('c')) or scanner_entry;peak=max(bars,key=lambda b:_f(b.get('h')));trough=min(bars,key=lambda b:_f(b.get('l')));last=bars[-1]
                peak_px=_f(peak.get('h'));trough_px=_f(trough.get('l'));close_px=_f(last.get('c'));ft=_iso(first.get('t'));pt=_iso(peak.get('t'));tt=_iso(trough.get('t'));complete=now>=close_utc
                drawdown=(close_px/peak_px-1)*100 if peak_px and close_px else None
                results.append({'id':row['id'],'peak_price':peak_px,'trough_price':trough_px,'close_price':close_px,'mfe_pct':(peak_px/entry-1)*100,'mae_pct':(trough_px/entry-1)*100,'close_return_pct':(close_px/entry-1)*100,'minutes_to_peak':(pt-captured).total_seconds()/60 if pt else None,'minutes_to_trough':(tt-captured).total_seconds()/60 if tt else None,'exit_price':close_px,'exit_return_pct':(close_px/entry-1)*100,'outcome_status':'complete' if complete else 'partial','outcome_bars':len(bars),'evaluated_at':now.isoformat(),'first_bar_time':ft.isoformat() if ft else None,'peak_time':pt.isoformat() if pt else None,'trough_time':tt.isoformat() if tt else None,'entry_bar_price':entry,'entry_slippage_pct':(entry/scanner_entry-1)*100,'drawdown_from_peak_pct':drawdown})
