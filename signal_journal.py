...
def _features(x,p,pmgap):
    return {'score':_f(x.get('score')),'price':_f(x.get('price')),'gap_pct':_f(x.get('gap_pct') if x.get('gap_pct') is not None else x.get('change')),
      'rvol':_f(x.get('rvol')),'momentum_pct':_f(x.get('move_to_1000_pct') if x.get('move_to_1000_pct') is not None else x.get('change')),
      'premarket_price':p.get('premarket_price'),'premarket_gap_pct':pmgap,'premarket_volume':p.get('premarket_volume'),'premarket_high':p.get('premarket_high'),'premarket_low':p.get('premarket_low')}
...
async def _bars(core,client,symbol,start,end,extended=True):
    ...
    params={'timeframe':'1Min','start':start,'end':end,'limit':10000,'feed':core.ALPACA_FEED,'adjustment':'split','sort':'asc'}
    ...
    async with httpx.AsyncClient(timeout=25) as client:
        for s in symbols:
            try:
                ...
                vol=sum(_f(b.get('v')) or 0 for b in bars); hi=max(_f(b.get('h')) or 0 for b in bars); lows=[_f(b.get('l')) for b in bars if (_f(b.get('l')) or 0)>0]
                out[s]={'premarket_price':_f(bars[-1].get('c')),'premarket_volume':vol,'premarket_high':hi or None,'premarket_low':min(lows) if lows else None}
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        for row in rows:
            try:
                ...
                bars=await _bars(core,client,row['symbol'],start.astimezone(timezone.utc).isoformat(),query_end.isoformat())
