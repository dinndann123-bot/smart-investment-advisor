...
async def _fetch(client,symbol,headers,feed,days):
    ...
    for raw in (r.json() or {}).get("bars") or []:
        try:
            b=_parse_bar(raw)
            if dtime(9,30)<=b["ts"].time()<dtime(16,0):grouped[b["ts"].date().isoformat()].append(b)
        except Exception:pass
    return [(d,sorted(x,key=lambda z:z["ts"])) for d,x in sorted(grouped.items()) if len(x)>=8],None

def _events(symbol,days):
    out=[]; history=[]; prev_close=None
    for date,bars in days:
        early=[b for b in bars if b["ts"].time()<dtime(10,0)]; post=[b for b in bars if b["ts"].time()>=dtime(10,0)]
        if len(early)<2 or len(post)<4:
            if bars:prev_close=bars[-1]["c"]
            continue
        early_vol=sum(b["v"] for b in early); baseline=statistics.median(history[-20:]) if len(history)>=8 else None; history.append(early_vol)
        open_px=early[0]["o"] or early[0]["c"]; px=early[-1]["c"]
        if not baseline or baseline<=0 or open_px<=0 or px<=0:prev_close=bars[-1]["c"]; continue
        gap=((open_px/prev_close)-1)*100 if prev_close else 0; move=(px/open_px-1)*100; rvol=early_vol/baseline
        strength=(px-min(b["l"] for b in early))/max(.01,max(b["h"] for b in early)-min(b["l"] for b in early)); volume_accel=early[-1]["v"]/max(1,statistics.mean([b["v"] for b in early[:-1]])) if len(early)>1 else 1; early_range=(max(b["h"] for b in early)/min(b["l"] for b in early)-1)*100; max_up=(max(b["h"] for b in post)/px-1)*100
...
def _fit(train):
    feats=["gap_pct","move_to_1000_pct","rvol","strength","volume_accel","early_range_pct"]; model={}; positives=[e for e in train if e["hit10"]]; negatives=[e for e in train if not e["hit10"]]
    for f in feats:
...
def _color(score): return "red" if score>=80 else "orange" if score>=65 else "yellow" if score>=50 else "gray"
def _summary(rows):
...
def install_explosion_learning(app):
    ...
    async def explosion_learning(days:int=Query(180,ge=120,le=365),symbols:int=Query(90,ge=50,le=len(UNIVERSE))):
        ...
        symbol_count=max(50,min(symbols,len(UNIVERSE))); cache_key=(max(180,days),symbol_count,feed,"timing-pattern-v2"); cached=_CACHE.get(cache_key)
        if cached:
            age=(datetime.now(timezone.utc)-cached["at"]).total_seconds()
            if age<=_CACHE_TTL_SECONDS:out=dict(cached["value"]); out["cached"]=True; out["cache_age_seconds"]=round(age,1); return out
        headers={"APCA-API-KEY-ID":key,"APCA-API-SECRET-KEY":secret}; universe=UNIVERSE[:symbol_count]; raw=[]; errors=[]; sem=asyncio.Semaphore(8)
        async with httpx.AsyncClient(timeout=45) as client:
            async def one(symbol):
                async with sem:
                    try:
                        ds,err=await _fetch(client,symbol,headers,feed,max(180,days)); return ([],err) if err else (_events(symbol,ds),None)
        ...
        split=dates[max(1,int(len(dates)*.70))]; train=[e for e in eligible if e["date"]<split]; test=[e for e in eligible if e["date"]>=split]; model=_fit(train)
        for e in train+test:e["explosion_score"],e["reasons"]=_score(e,model); e["color"]=_color(e["explosion_score"])
        ...
        timing_train=summarize_timing(train); timing_holdout=summarize_timing(test); timing_patterns=summarize_timing_patterns(train,test,min_samples=20)
        out={"ok":True,"method":"explosion-pattern chronological holdout + segmented timing-v2","cutoff_ny":"10:00","feed":feed,"split_date":split,"samples":{"total":len(eligible),"train":len(train),"test":len(test)},"baseline_test":_summary(test),"color_buckets_test":_bucket(test),"feature_importance_train_only":feature_importance,"timing":{"entry_windows_ny":["09:30","09:35","09:45","10:00"],"train":timing_train,"holdout":timing_holdout,"patterns":timing_patterns,"score_impact":"none until holdout validation"},"largest_holdout_explosions":sorted(test,key=lambda e:e["max_up_pct"],reverse=True)[:25],"strongest_holdout_signals":sorted(test,key=lambda e:e["explosion_score"],reverse=True)[:25],"error_count":len(errors),"errors":errors[:20],"cached":False,"limitations":["המודל לומד רק מידע שהיה זמין עד 10:00 ניו יורק לציון ההתפוצצות.","ספי Gap/RVOL/מומנטום לתבניות נלמדים מתקופת train בלבד.","מדדי התזמון והתבניות אינם משנים את הציון עד לאימות holdout.","היקום עדיין קבוע ואינו כל השוק האמריקאי.","חדשות, float ו-short interest עדיין אינם כלולים.","ב-Feed מסוג IEX הנתונים אינם מייצגים את כל השוק המאוחד; SIP מדויק יותר כאשר זמין."]}
