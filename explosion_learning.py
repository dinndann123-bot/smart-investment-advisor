...
def _default_headers():
    return {"APCA-API-KEY-ID":os.getenv("ALPACA_API_KEY",os.getenv("APCA_API_KEY_ID","")),"APCA-API-SECRET-KEY":os.getenv("ALPACA_SECRET_KEY",os.getenv("APCA_API_SECRET_KEY",""))}
def _default_feed(): return os.getenv("ALPACA_FEED","iex")
...
def _events(symbol,days):
    ...
    for date,bars in days:
        pm=[b for b in bars if dtime(4,0)<=b["ts"].time()<dtime(9,30)]; regular=[b for b in bars if dtime(9,30)<=b["ts"].time()<dtime(16,0)]; early=[b for b in regular if b["ts"].time()<dtime(10,0)]; post=[b for b in regular if b["ts"].time()>=dtime(10,0)]
        if len(early)<2 or len(post)<4:
            if regular:prev_close=regular[-1]["c"]
            continue
        early_vol=sum(b["v"] for b in early); baseline=statistics.median(early_history[-20:]) if len(early_history)>=8 else None; early_history.append(early_vol); open_px=early[0]["o"] or early[0]["c"]; px=early[-1]["c"]
        ...
        gap=((open_px/prev_close)-1)*100 if prev_close else 0; move=(px/open_px-1)*100; rvol=early_vol/baseline; strength=(px-min(b["l"] for b in early))/max(.01,max(b["h"] for b in early)-min(b["l"] for b in early)); volume_accel=early[-1]["v"]/max(1,statistics.mean([b["v"] for b in early[:-1]])); early_range=(max(b["h"] for b in early)/min(b["l"] for b in early)-1)*100; max_up=(max(b["h"] for b in post)/px-1)*100
        pm_last=pm[-1]["c"] if pm else open_px; pm_first=(pm[0]["o"] or pm[0]["c"]) if pm else open_px; pm_high=max((b["h"] for b in pm),default=open_px); pm_low=min((b["l"] for b in pm),default=open_px); pm_gap=((pm_last/prev_close)-1)*100 if prev_close and pm else gap; pm_range=((pm_high/pm_low)-1)*100 if pm_low>0 and pm else 0; pm_position=(pm_last-pm_low)/max(.01,pm_high-pm_low) if pm else .5; pm_momentum=((pm_last/pm_first)-1)*100 if pm_first>0 and pm else 0; pm_rvol=(pm_vol/pm_baseline) if pm_baseline and pm_baseline>0 else 1.0
        out.append({"symbol":symbol,"date":date,"price":px,"gap_pct":gap,"move_to_1000_pct":move,"rvol":rvol,"strength":strength,"volume_accel":volume_accel,"early_range_pct":early_range,"premarket_gap_pct":pm_gap,"premarket_rvol":pm_rvol,"premarket_range_pct":pm_range,"premarket_position":pm_position,"premarket_momentum_pct":pm_momentum,"premarket_volume":pm_vol,"premarket_bars":len(pm),"max_up_pct":max_up,"est_daily_volume":baseline*13,"hit10":int(max_up>=10),"hit20":int(max_up>=20),"hit30":int(max_up>=30),"timing_windows":measure_day_timing(regular)}); prev_close=regular[-1]["c"]
    return out
...
def _score(e,model):
    ...
    for f,m in model.items():
        z=(e[f]-m["median"])/(1.4826*m["mad"] or 1); aligned=max(-2.5,min(2.5,z*m["direction"])); component=(aligned+2.5)/5*100; total+=component*m["weight"]; reasons.append({"feature":f,"value":round(e[f],3),"weight":round(m["weight"],4),"direction":"higher" if m["direction"]>0 else "lower"})
...
async def install_explosion_learning(app,headers_fn=None,feed_fn=None):
    ...
    async def explosion_research(days:int=Query(180,ge=60,le=3650),symbols:int=Query(60,ge=10,le=len(UNIVERSE)),refresh:bool=False):
        feed=feed_fn(); key=(days,symbols,feed); cached=_CACHE.get(key)
        ...
        features=["gap_pct","move_to_1000_pct","rvol","strength","volume_accel","early_range_pct","premarket_gap_pct","premarket_rvol","premarket_range_pct","premarket_position","premarket_momentum_pct"]; validation=_walk_forward(events,features); payload={"generated_at":datetime.now(timezone.utc).isoformat(),"feed":feed,"days_requested":days,"symbols_requested":symbols,"events":len(events),"validation":validation,"timing_summary":summarize_timing(events),"timing_patterns":summarize_timing_patterns(events),"errors":errors[:20]}; _CACHE[key]=(datetime.now(timezone.utc),payload);return payload
