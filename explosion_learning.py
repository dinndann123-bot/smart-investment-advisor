...
async def _fetch(client, symbol, headers, feed, days):
    ...
    for raw in (r.json() or {}).get("bars") or []:
        try:
            b = _parse_bar(raw)
            if dtime(4,0) <= b["ts"].time() < dtime(16,0):
...
def _events(symbol, days):
    ...
    for date,bars in days:
        ...
        early=[b for b in regular if b["ts"].time()<dtime(10,0)]
        post=[b for b in regular if b["ts"].time()>=dtime(10,0)]
        ...
        if not baseline or baseline<=0 or open_px<=0 or px<=0:
            prev_close=regular[-1]["c"]; continue
        gap=((open_px/prev_close)-1)*100 if prev_close else 0; move=(px/open_px-1)*100; rvol=early_vol/baseline
        strength=(px-min(b["l"] for b in early))/max(.01,max(b["h"] for b in early)-min(b["l"] for b in early))
        volume_accel=early[-1]["v"]/max(1,statistics.mean([b["v"] for b in early[:-1]])); early_range=(max(b["h"] for b in early)/min(b["l"] for b in early)-1)*100; max_up=(max(b["h"] for b in post)/px-1)*100
        ...
        pm_gap=((pm_last/prev_close)-1)*100 if prev_close and pm else gap; pm_range=((pm_high/pm_low)-1)*100 if pm_low>0 and pm else 0; pm_position=(pm_last-pm_low)/max(.01,pm_high-pm_low) if pm else .5; pm_momentum=((pm_last/pm_first)-1)*100 if pm_first>0 and pm else 0; pm_rvol=(pm_vol/pm_baseline) if pm_baseline and pm_baseline>0 else 1.0
...
def _robust(xs):
    med=statistics.median(xs); mad=statistics.median([abs(x-med) for x in xs]) or 1e-9
    return med,mad

def _fit(train, features):
    model={}; positives=[e for e in train if e["hit10"]]; negatives=[e for e in train if not e["hit10"]]
    for f in features:
        vals=[e[f] for e in train]; med,mad=_robust(vals); p=_safe_mean([e[f] for e in positives]) or med; n=_safe_mean([e[f] for e in negatives]) or med; separation=abs(p-n)/(mad or 1)
...
def _score(e, model):
    ...
    for f,m in model.items():
        z=(e[f]-m["median"])/(1.4826*m["mad"] or 1); aligned=max(-2.5,min(2.5,z*m["direction"])); total+=((aligned+2.5)/5*100)*m["weight"]
...
def _walk_forward(events, features):
    ...
    model=_fit(train,features); scored=sorted([(_score(e,model),e) for e in test],key=lambda x:x[0],reverse=True); top=scored[:max(10,int(len(scored)*.2))]
...
def install_explosion_learning(app, headers_fn=None, feed_fn=None):
    """Install only the explosion-research route. Core search/long/journal modules are installed by the app composition and must not be re-installed here."""
    headers_fn=headers_fn or _default_headers; feed_fn=feed_fn or _default_feed
    if any(getattr(r,"path",None)=="/api/strategy/explosions" for r in app.routes):
