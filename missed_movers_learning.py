...
def install_missed_movers_learning(app,core):
    ...
    async def missed_movers(threshold_pct:float=MOVE_THRESHOLD_PCT,limit:int=100):
        ...
        for r in con.execute('SELECT symbol,MIN(rank) best_rank,MIN(captured_at) first_seen FROM signal_journal WHERE trade_date=? AND strategy_version=? GROUP BY symbol',(day,STRATEGY_VERSION)).fetchall():
            top[r['symbol']]={'rank':int(r['best_rank']),'first_seen':r['first_seen']}
        ...
        for x in movers[:max(1,min(limit,50))]:
            symbol=x['symbol']; asset=await _asset(core,symbol); itype=_instrument(asset,symbol); eligible=_eligible(asset,itype); hit=top.get(symbol)
            if not eligible:excluded[itype]=excluded.get(itype,0)+1
            if eligible:
                eligible_movers+=1
                if hit:eligible_overlap+=1
            snap=await _iex_snapshot(core,symbol); verified=bool(snap)
            if verified:verified_count+=1
            pm=await _premarket(core,symbol,day) if verified and eligible else {}; prev=_f((snap or {}).get('prevDailyBar',{}).get('c')); pmgap=((pm.get('last')/prev)-1)*100 if pm.get('last') and prev else None; attrs=(asset or {}).get('attributes') or []
