...
def _db():
    ...
    for col,typ in {'premarket_gap_pct':'REAL','premarket_volume':'REAL','premarket_high':'REAL','premarket_low':'REAL','premarket_last':'REAL','first_seen_top10':'TEXT','discovery_source':'TEXT','verification_feed':'TEXT','verified':'INTEGER','asset_name':'TEXT','instrument_type':'TEXT','eligible_common_stock':'INTEGER','asset_status':'TEXT','tradable':'INTEGER','asset_attributes':'TEXT'}.items():
...
async def _sip_movers(core,limit=50):
    ...
    h={'APCA-API-KEY-ID':core.ALPACA_KEY,'APCA-API-SECRET-KEY':core.ALPACA_SECRET}; requested_top=max(1,min(int(limit or 50),50))
    async with httpx.AsyncClient(timeout=30) as c:r=await c.get('https://data.alpaca.markets/v1beta1/screener/stocks/movers',headers=h,params={'top':requested_top})
    if r.status_code>=400:return [],{'status':f'http_{r.status_code}','count':0,'requested_top':requested_top,'body':(r.text or '')[:200]}
    ...
    out.sort(key=lambda z:z['move_pct'],reverse=True);return out,{'status':'ok','count':len(out),'raw_gainers':len(raw),'requested_top':requested_top,'keys':list(data.keys())[:10]}
...
async def _asset(core,symbol):
    ...
    try:
        async with httpx.AsyncClient(timeout=12) as c:r=await c.get(f'https://paper-api.alpaca.markets/v2/assets/{symbol}',headers=h)
        return (r.json() or {}) if r.status_code<400 else None
    except Exception:return None

def _instrument(asset,symbol):
    name=str((asset or {}).get('name') or '').lower();s=str(symbol or '').upper()
    if 'warrant' in name or s.endswith('.WS') or s.endswith('WS') or (len(s)>=5 and s.endswith('W')):return 'warrant'
    if 'right' in name or s.endswith('.RT') or s.endswith('RT') or (len(s)>=5 and s.endswith('R')):return 'right'
    if 'unit' in name or s.endswith('.U'):return 'unit'
    if 'preferred' in name or 'depositary share' in name:return 'preferred'
    if 'etf' in name or 'exchange traded fund' in name:return 'etf'
    if 'common stock' in name or 'common share' in name or 'ordinary share' in name:return 'common_stock'
    return 'other_equity'
...
def install_missed_movers_learning(app,core):
    ...
    async def missed_movers(threshold_pct:float=MOVE_THRESHOLD_PCT,limit:int=100):
        ...
        movers=[x for x in discovered if x['move_pct']>=threshold_pct];saved=0;missed=[];eligible_movers=0;eligible_overlap=0;excluded={};verified_count=0
        for x in movers[:max(1,min(limit,50))]:
            symbol=x['symbol'];asset=await _asset(core,symbol);itype=_instrument(asset,symbol);eligible=_eligible(asset,itype);excluded[itype]=excluded.get(itype,0)+(0 if eligible else 1);hit=top.get(symbol)
            if eligible:eligible_movers+=1;eligible_overlap+=1 if hit else 0
            snap=await _iex_snapshot(core,symbol);verified=bool(snap);verified_count+=1 if verified else 0;pm=await _premarket(core,symbol,day) if verified and eligible else {};prev=_f((snap or {}).get('prevDailyBar',{}).get('c'));pmgap=((pm.get('last')/prev)-1)*100 if pm.get('last') and prev else None;attrs=(asset or {}).get('attributes') or []
            features={'point_in_time_rule':'SIP discovers outcome movers; asset reference classifies instrument; only active tradable common/ordinary equity counts in stock capture rate','sip_mover':x['raw'],'asset':asset,'iex_snapshot':snap,'premarket':pm}
            con.execute('INSERT OR REPLACE INTO missed_movers(trade_date,symbol,observed_at,move_pct,price,volume,was_top10,top10_best_rank,strategy_version,features,premarket_gap_pct,premarket_volume,premarket_high,premarket_low,premarket_last,first_seen_top10,discovery_source,verification_feed,verified,asset_name,instrument_type,eligible_common_stock,asset_status,tradable,asset_attributes) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(day,symbol,now.isoformat(),x['move_pct'],x['price'],_f((snap or {}).get('dailyBar',{}).get('v')),1 if hit else 0,hit['rank'] if hit else None,STRATEGY_VERSION,json.dumps(features,ensure_ascii=False),pmgap,pm.get('volume'),pm.get('high'),pm.get('low'),pm.get('last'),hit['first_seen'] if hit else None,'alpaca_sip_screener',core.ALPACA_FEED,1 if verified else 0,(asset or {}).get('name'),itype,1 if eligible else 0,(asset or {}).get('status'),1 if (asset or {}).get('tradable') is True else 0,json.dumps(attrs));saved+=1
            if eligible and not hit:missed.append({'symbol':symbol,'name':(asset or {}).get('name'),'move_pct':round(x['move_pct'],2),'instrument_type':itype,'verified_iex':verified,'overnight_halted':'overnight_halted' in attrs,'premarket_gap_pct':round(pmgap,2) if pmgap is not None else None,'premarket_volume':pm.get('volume'),'premarket_range_pct':round((pm['high']/pm['low']-1)*100,2) if pm.get('high') and pm.get('low') else None})
        con.commit();con.close();eligible_missed=max(0,eligible_movers-eligible_overlap)
        return {'ok':True,'trade_date':day,'threshold_pct':threshold_pct,'raw_market_movers':len(movers),'eligible_stock_movers':eligible_movers,'eligible_top10_overlap':eligible_overlap,'eligible_false_negatives':eligible_missed,'eligible_capture_rate_pct':round(eligible_overlap/eligible_movers*100,1) if eligible_movers else None,'excluded_instruments':excluded,'verified_iex':verified_count,'missed':missed[:25],'saved':saved,'strategy_version':STRATEGY_VERSION,'discovery_source':'alpaca_sip_screener','discovery_diag':diag,'verification_feed':core.ALPACA_FEED,'point_in_time':True}

    @app.get('/api/learning/missed-movers/summary')
    async def missed_summary(days:int=30):
        con=_db();rows=con.execute('''SELECT trade_date,COUNT(*) raw_movers,SUM(CASE WHEN eligible_common_stock=1 THEN 1 ELSE 0 END) eligible_movers,SUM(CASE WHEN eligible_common_stock=1 AND was_top10=1 THEN 1 ELSE 0 END) captured,SUM(CASE WHEN eligible_common_stock=1 AND was_top10=0 THEN 1 ELSE 0 END) missed,SUM(CASE WHEN eligible_common_stock=0 THEN 1 ELSE 0 END) excluded,AVG(CASE WHEN eligible_common_stock=1 AND was_top10=0 THEN premarket_gap_pct END) missed_avg_pm_gap,AVG(CASE WHEN eligible_common_stock=1 AND was_top10=0 THEN premarket_volume END) missed_avg_pm_volume FROM missed_movers WHERE strategy_version=? GROUP BY trade_date ORDER BY trade_date DESC LIMIT ?''',(STRATEGY_VERSION,max(1,min(days,365)))).fetchall();con.close();out=[dict(r) for r in rows];eligible=sum(int(x['eligible_movers'] or 0) for x in out);captured=sum(int(x['captured'] or 0) for x in out)
