...
def install_learning_comparison(app,core):
    ...
    async def feature_comparison():
        ...
        quality_flags=[];clean=[]
        for r in mm:
            move=float(r['move_pct'] or 0)
            if abs(move)>=EXTREME_MOVE_PCT:
                quality_flags.append({'symbol':r['symbol'],'move_pct':round(move,2),'flag':'extreme_move_requires_review','excluded_from_feature_aggregates':True})
            else:clean.append(r)
        captured=[r for r in clean if r['was_top10']==1];missed=[r for r in clean if r['was_top10']==0]
        def group(rows):
