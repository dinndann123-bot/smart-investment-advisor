...
This module deliberately contains no urllib/exec loading.  It installs only the
scanner route and keeps the existing selection thresholds unchanged.
"""
...
STRATEGY_VERSION='strategy-learning-v6.6.6-local-quality-guard'
...
def install_local_scanner(app):
    ...
    feed=(os.getenv('ALPACA_FEED') or 'iex').strip()
    def f(v,d=0.0):
        try:return float(v)
        except:return d
    def hdr():return {'APCA-API-KEY-ID':key,'APCA-API-SECRET-KEY':secret}
    async def assets():
        # Asset metadata is a trading API concern; use the configured paper account directly.
        async with httpx.AsyncClient(timeout=15) as c:
            r=await c.get('https://paper-api.alpaca.markets/v2/assets',headers=hdr(),params={'asset_class':'us_equity','status':'active'})
        out=[]
        if r.status_code<400:
            ...
            for a in r.json() or []:
                ...
                if any(x in n for x in bad):continue
    ...
    def qrank(r):
        ch=f(r.get('change_pct'));gap=f(r.get('snapshot_gap_pct'));vr=f(r.get('snapshot_volume_ratio'));rp=f(r.get('snapshot_range_position'),.5);dv=f(r.get('price'))*f(r.get('day_volume'))
    ...
    def enrich(r,bs):
        ...
        baseline=sum(hist[-5:])/len(hist[-5:]) if hist else 0;live=f(r.get('day_volume'));raw=live/baseline if baseline else f(r.get('snapshot_volume_ratio'))
        reliable=bool(baseline>=1000 and live>=1000);rvol=min(raw,25.0) if raw>0 else 0
        p=f(r.get('price'));lo=f(r.get('day_low'),p);hi=f(r.get('day_high'),p);used=(p-lo)/(hi-lo)*100 if hi>lo else 50;rp=f(r.get('snapshot_range_position'),.5);mv=f(r.get('minute_volume'));avg_min=(baseline/max((cur-570)/5,1)) if baseline else 0;burst=mv/avg_min if avg_min else 0
        r.update({'bars_count':len(bs),'today_bars_count':len(todays),'rvol_raw':round(raw,2) if raw else None,'rvol':round(rvol,2) if rvol else None,'rvol_reliable':reliable,'rvol_capped':bool(raw>25),'intraday_move_used_pct':round(used,1),'minute_volume_burst':round(burst,2) if burst else None,'historical_baseline_volume':round(baseline) if baseline else None,'current_range_position':round(rp,3),'live_structure_source':'snapshot+minuteBar'});return r
    def stage(r):
    ...
    async def scanner(top:int=10,candidates:int=40):
        ...
        print(f'LOCAL_SCANNER_START scan_id={sid} feed={feed}',flush=True)
        ...
        async with httpx.AsyncClient(timeout=7,limits=httpx.Limits(max_connections=30,max_keepalive_connections=20)) as c:
            ...
            for cap in (80,120,180):
                ...
                print(f'LOCAL_SCANNER_EXPAND scan_id={sid} deep={len(en)} predictive={len(pred)} watch={len(watch)} extended={len(ext)} cap={cap}',flush=True)
                if len(pred)>=wanted:break
        sel=pred[:wanted];payload={'results':sel,'watch_observations':sorted(watch,key=lambda x:x['forward_rank'],reverse=True)[:30],'extended_observations':ext[:30],'diagnostic_sample':en[:5],'assets_scanned':len(aa),'full_market_ranked_count':len(pool),'deep_candidates':len(en),'forward_candidate_count':len(pred),'requested_top':wanted,'complete_top10':len(sel)>=wanted,'strategy_version':STRATEGY_VERSION,'scan_id':sid,'generated_at':ts.isoformat(),'data_source':'Alpaca','feed':feed,'ranking_status':'hybrid_live_predictive_only','candidate_semantics':'prebreakout_or_earlybreakout_only','cache_policy':'no-store','quality_guard':{'common_share_symbol_filter':True,'rvol_cap':25,'minimum_baseline_volume':1000,'unreliable_rvol_excluded_from_predictive':True}}
        print(f'LOCAL_SCANNER_DONE scan_id={sid} results={len(sel)} predictive={len(pred)} complete={payload["complete_top10"]}',flush=True);return JSONResponse(payload,headers={'Cache-Control':'no-store','X-Scanner-Version':STRATEGY_VERSION,'X-Scan-Id':sid})
    ...
    app.add_api_route('/api/scanner/day',scanner,methods=['GET'],name='scanner_day_local')
    print('LOCAL_SCANNER_INSTALLED no_runtime_exec=true',flush=True)
    return scanner
