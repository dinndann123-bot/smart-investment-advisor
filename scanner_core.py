"""Local scanner core: session-aware feed selection + fresh snapshots + batched bars."""
import asyncio, math, os, re, uuid
from datetime import datetime, timezone, timedelta
from zoneinfo import ZoneInfo
import httpx
from fastapi.responses import JSONResponse
NY=ZoneInfo('America/New_York'); STRATEGY_VERSION='strategy-learning-v6.7.0-session-feed-delayed-sip'
def install_local_scanner(app):
 ...
 def session(now):
  ...
  return 'closed'
 def scan_feed(now):
  # Free IEX can be stale outside the regular session. delayed_sip provides consolidated
  # US-market coverage with a documented 15-minute delay and includes extended-hours bars.
  return 'delayed_sip' if session(now) in {'premarket','afterhours'} else regular_feed
 def fresh(ts,now,feed):
  z=parse_ts(ts)
  if not z or z.date()!=now.date():return False
  # delayed_sip is intentionally delayed; accept today's timestamp rather than imposing a live-age check.
  return True
 async def assets():
 ...
 async def batch_bars(symbols,c,feed):
  out={s:([],None) for s in symbols}; end=datetime.now(timezone.utc); start=end-timedelta(days=8)
  # With delayed SIP, cap end at 15 minutes ago to remain outside the restricted recent-SIP window.
  if feed=='delayed_sip':end=end-timedelta(minutes=15)
  for i in range(0,len(symbols),50):
   ch=symbols[i:i+50]; token=None; pages=0
   while pages<6:
    params={'symbols':','.join(ch),'timeframe':'5Min','start':start.isoformat().replace('+00:00','Z'),'end':end.isoformat().replace('+00:00','Z'),'feed':feed,'adjustment':'split','limit':10000,'sort':'asc'}
    ...
    try:r=await c.get('https://data.alpaca.markets/v2/stocks/bars',headers=hdr(),params=params)
     ...
     break
    if r.status_code>=400:
     for s in ch:out[s]=(out[s][0],f'HTTP_{r.status_code}')
     break
 ...
 def enrich(r,bar_result):
  ...
  todays=by.get(now.date(),[]); cur=now.hour*60+now.minute; sess=session(now); session_start=240 if sess=='premarket' else 570
  # delayed SIP can only be evaluated through now-15m; compare historical sessions at the same effective clock.
  effective_cur=cur-(15 if r.get('data_feed')=='delayed_sip' else 0); effective_cur=max(session_start,effective_cur)
  ...
  highs=[f(b.get('h')) for z,b in todays if session_start<=z.hour*60+z.minute<=effective_cur]; lows=[f(b.get('l')) for z,b in todays if session_start<=z.hour*60+z.minute<=effective_cur and f(b.get('l'))>0]
 ...
 async def scanner(top:int=10,candidates:int=40):
  ts=datetime.now(timezone.utc); now=ts.astimezone(NY); sess=session(now); feed=scan_feed(now); sid=f"{ts.strftime('%Y%m%dT%H%M%SZ')}-{uuid.uuid4().hex[:8]}"; wanted=10; print(f'LOCAL_SCANNER_START scan_id={sid} session={sess} feed={feed}',flush=True)
  ...
  async with httpx.AsyncClient(timeout=20,limits=httpx.Limits(max_connections=8,max_keepalive_connections=8)) as c:
   for cap in (80,120,180):
    ...
    pred.sort(key=lambda x:({'pre_breakout':2,'early_breakout':1}.get(x['breakout_stage'],0),x['forward_rank']),reverse=True);print(f'LOCAL_SCANNER_EXPAND scan_id={sid} session={sess} feed={feed} fresh_pool={len(pool)} stale_filtered={stale} deep={len(en)} predictive={len(pred)} watch={len(watch)} extended={len(ext)} cap={cap}',flush=True)
    if len(pred)>=wanted:break
  diag=sorted(en,key=lambda x:(f(x.get('rvol_raw')),-f(x.get('bars_count'))),reverse=True)[:10]
  for d in diag[:5]:print(f"MARKET_DATA_DIAG {d.get('ticker')} feed={d.get('data_feed')} bars={d.get('bars_count')} err={d.get('bars_error')} today={d.get('today_bars_count')} hist={d.get('historical_days_with_volume')} livevol={d.get('session_live_volume')} baseline={d.get('historical_baseline_volume')} rvol={d.get('rvol_raw')} reliable={d.get('rvol_reliable')} px={d.get('price')} ts={d.get('market_timestamp')}",flush=True)
  sel=pred[:wanted];payload={'results':sel,'watch_observations':sorted(watch,key=lambda x:x['forward_rank'],reverse=True)[:30],'extended_observations':ext[:30],'diagnostic_sample':diag,'assets_scanned':len(aa),'snapshots_received':len(ss),'stale_snapshots_filtered':stale,'fresh_market_pool':len(pool),'deep_candidates':len(en),'forward_candidate_count':len(pred),'requested_top':wanted,'complete_top10':len(sel)>=wanted,'strategy_version':STRATEGY_VERSION,'scan_id':sid,'generated_at':ts.isoformat(),'session':sess,'data_source':'Alpaca','feed':feed,'data_delay_minutes':15 if feed=='delayed_sip' else 0,'ranking_status':'session-aware-predictive','candidate_semantics':'prebreakout_or_earlybreakout_only','cache_policy':'no-store','quality_guard':{'fresh_snapshot_required':True,'batched_bars':True,'session_feed_selection':True,'delayed_sip_premarket':True,'rvol_cap':25,'minimum_baseline_volume':1000,'unreliable_rvol_excluded_from_predictive':True,'same_effective_clock_baseline':True}}
  print(f'LOCAL_SCANNER_DONE scan_id={sid} session={sess} feed={feed} assets={len(aa)} snaps={len(ss)} fresh={len(pool)} stale_filtered={stale} results={len(sel)} predictive={len(pred)} complete={payload["complete_top10"]}',flush=True);return JSONResponse(payload,headers={'Cache-Control':'no-store','X-Scanner-Version':STRATEGY_VERSION,'X-Scan-Id':sid,'X-Market-Feed':feed})
 ...
 app.add_api_route('/api/scanner/day',scanner,methods=['GET'],name='scanner_day_local');print('LOCAL_SCANNER_INSTALLED no_runtime_exec=true session_feed=true delayed_sip_premarket=true batched_bars=true',flush=True);return scanner
