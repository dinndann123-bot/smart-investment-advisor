...
NY=ZoneInfo('America/New_York'); STRATEGY_VERSION='strategy-learning-v6.6.7-local-premarket-rvol'
def install_local_scanner(app):
 ...
 def enrich(r,bs):
  ...
  todays=by.get(now.date(),[]); cur=now.hour*60+now.minute
  # Match historical volume to the SAME market session/time. During premarket use 04:00->now;
  # regular hours use 09:30->now. This avoids the old impossible 09:30->07:xx baseline.
  session_start=240 if cur<570 else 570
  live=sum(f(b.get('v')) for z,b in todays if session_start<=z.hour*60+z.minute<=cur)
  hist=[]
  ...
  prices=[f(b.get('c')) for z,b in todays if session_start<=z.hour*60+z.minute<=cur and f(b.get('c'))>0]; highs=[f(b.get('h')) for z,b in todays if session_start<=z.hour*60+z.minute<=cur]; lows=[f(b.get('l')) for z,b in todays if session_start<=z.hour*60+z.minute<=cur and f(b.get('l'))>0]
