"""Local scanner core: fresh-session snapshots + batched intraday bars."""
import asyncio, math, os, re, uuid
from datetime import datetime, timezone, timedelta
from zoneinfo import ZoneInfo
import httpx
from fastapi.responses import JSONResponse
NY=ZoneInfo('America/New_York'); STRATEGY_VERSION='strategy-learning-v6.6.9-fresh-batched-bars'
def install_local_scanner(app):
 ...
 def fresh(ts,now):
  ...
  # Current US trading date is mandatory. This rejects inactive symbols whose snapshot
  # is weeks/months old before they can consume deep-scan capacity.
  return z.date()==now.date()
 async def assets():
 ...
 async def batch_bars(symbols,c):
  out={s:([],None) for s in symbols}; end=datetime.now(timezone.utc); start=end-timedelta(days=8)
  # Alpaca multi-symbol bars endpoint: one request per chunk rather than one request per stock.
 ...
 async def scanner(top:int=10,candidates:int=40):
  ts=datetime.now(timezone.utc); now=ts.astimezone(NY); sid=f"{ts.strftime('%Y%m%dT%H%M%SZ')}-{uuid.uuid4().hex[:8]}"; wanted=10; print(f'LOCAL_SCANNER_START scan_id={sid} feed={feed}',flush=True)
  aa=await assets();ss=await snaps([x['ticker'] for x in aa]);pool=[];stale=0
  for a in aa:
   if a['ticker'] in ss:
    x=basic(a,ss[a['ticker']],now)
    if x:pool.append(x)
    else:stale+=1
  ...
  async with httpx.AsyncClient(timeout=20,limits=httpx.Limits(max_connections=8,max_keepalive_connections=8)) as c:
   for cap in (80,120,180):
    batch=[x for x in pool[:min(cap,len(pool))] if x['ticker'] not in seen]
    bm=await batch_bars([x['ticker'] for x in batch],c) if batch else {}
    rows=[enrich(dict(x),bm.get(x['ticker'],([],None))) for x in batch]
    for x in rows:
    ...
    pred.sort(key=lambda x:({'pre_breakout':2,'early_breakout':1}.get(x['breakout_stage'],0),x['forward_rank']),reverse=True);print(f'LOCAL_SCANNER_EXPAND scan_id={sid} fresh_pool={len(pool)} stale_filtered={stale} deep={len(en)} predictive={len(pred)} watch={len(watch)} extended={len(ext)} cap={cap}',flush=True)
  ...
  sel=pred[:wanted];payload={'results':sel,'watch_observations':sorted(watch,key=lambda x:x['forward_rank'],reverse=True)[:30],'extended_observations':ext[:30],'diagnostic_sample':diag,'assets_scanned':len(aa),'snapshots_received':len(ss),'stale_snapshots_filtered':stale,'fresh_market_pool':len(pool),'deep_candidates':len(en),'forward_candidate_count':len(pred),'requested_top':wanted,'complete_top10':len(sel)>=wanted,'strategy_version':STRATEGY_VERSION,'scan_id':sid,'generated_at':ts.isoformat(),'data_source':'Alpaca','feed':feed,'ranking_status':'fresh-session-predictive','candidate_semantics':'prebreakout_or_earlybreakout_only','cache_policy':'no-store','quality_guard':{'fresh_snapshot_required':True,'batched_bars':True,'rvol_cap':25,'minimum_baseline_volume':1000,'unreliable_rvol_excluded_from_predictive':True,'premarket_same_clock_baseline':True}}
  print(f'LOCAL_SCANNER_DONE scan_id={sid} assets={len(aa)} snaps={len(ss)} fresh={len(pool)} stale_filtered={stale} results={len(sel)} predictive={len(pred)} complete={payload["complete_top10"]}',flush=True);return JSONResponse(payload,headers={'Cache-Control':'no-store','X-Scanner-Version':STRATEGY_VERSION,'X-Scan-Id':sid})
 ...
 app.add_api_route('/api/scanner/day',scanner,methods=['GET'],name='scanner_day_local');print('LOCAL_SCANNER_INSTALLED no_runtime_exec=true fresh_snapshot_filter=true batched_bars=true',flush=True);return scanner
