...
- סורק המסחר היומי כבר לא מחזיר 503 כאשר Alpaca אינו מוגדר. הוא עובר ל-Alpha Vantage אם קיים, ולאחר מכן לסריקת גיבוי ציבורית מסומנת שאינה סריקת כל השוק.
- גרפים משתמשים בשרשרת ספקים: Alpaca -> Alpha Vantage -> Yahoo chart fallback -> Stooq לגרפים היסטוריים.
- גרף 1D עובר אוטומטית לגרף חודש כאשר נתוני intraday אינם זמינים.
- גרף 5 שנים משתמש בנתונים יומיים ומקור היסטורי שני, ונשמר במטמון לאחר טעינה מוצלחת.
- נתונים מוצלחים נשמרים ב-market_cache; תקלה זמנית בספק לא מוחקת גרף שכבר נטען בעבר.
- תוקנה שגיאת JavaScript במסך 5Y ותוקנה שגיאת fallback בסורק.
- נבדקו ב-QA נתיבי 1D/1M/1Y/5Y, פרופיל 5Y, הסורק ושעוני ישראל/ניו-יורק.

לנתוני זמן אמת אמיתיים יש להגדיר Alpaca ב-.env. המקורות הציבוריים הם fallback ואינם התחייבות ל-feed בזמן אמת.

## Web App / PWA — עדכונים ללא הורדת ZIP מחדש

הפרויקט מוכן כעת להתקנה כ-Web App/PWA. לאחר פריסה לשרת עם HTTPS:

- פותחים כתובת אחת קבועה בדפדפן.
- בטלפון אפשר לבחור "הוספה למסך הבית" ולהשתמש בה כאפליקציה.
- עדכון שמועלה לשרת מופיע אוטומטית; אין צורך להוריד ZIP חדש.
- Service Worker בודק גרסה חדשה, מציג הודעת "עדכן עכשיו", וטוען את הגרסה החדשה.
- בקשות `/api/*` אינן נשמרות ב-cache כדי לא להציג נתוני שוק ישנים.
- `index.html` ו-Service Worker מוגשים עם `no-cache` כדי למנוע היתקעות על ממשק ישן.

### פריסה רציפה

נוסף `Dockerfile` וכן `render.yaml` לדוגמה. כאשר הפרויקט נמצא ב-Git repository שמחובר ל-Render (או שירות מקביל), כל push לענף הפריסה יכול לבצע Auto Deploy. מרגע זה משתמשי האפליקציה לא מורידים קבצים מחדש.

### סודות בענן

בפריסה לענן אין להעלות `.env`. יש להזין את `ALPACA_API_KEY`, `ALPACA_SECRET_KEY` ו-`ALPHA_VANTAGE_API_KEY` בממשק Environment Variables של ספק האחסון.

הערה: מסך השמירה המקומי של Alpaca כותב ל-`.env`, ולכן בפריסה עננית מומלץ להגדיר מפתחות דרך Environment Variables ולא להסתמך על כתיבה מקומית שנעלמת בפריסה מחדש.
