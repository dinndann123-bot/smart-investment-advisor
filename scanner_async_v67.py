...
STRATEGY_VERSION = "strategy-learning-v6.7-async-scanner-cache"


def install_async_scanner(app, scanner_engine):
    """Wrap an existing async scanner endpoint with a non-blocking single-flight job/cache API."""
    state = {
        "status": "idle",
        "job_id": None,
        "started_at": None,
    ...
    async def run_scan(job_id, top, candidates):
        async with lock:
            ...
            try:
                response = await scanner_engine(top=top, candidates=candidates)
                ...
                print(
                    f"SCANNER_V67_JOB_DONE job_id={job_id} results={len(payload.get('results') or [])} duration={state['duration_sec']}",
                    flush=True,
                )
                ...
                print(f"SCANNER_V67_JOB_ERROR job_id={job_id} error={state['error']}", flush=True)
